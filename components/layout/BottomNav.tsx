...
export function BottomNav({ activeTab, onTabChange, className = '' }: BottomNavProps) {
  ...
  return (
    <nav className={`border-t border-[#493222] bg-[#221710] px-2 pt-2 pb-6 ${className}`}>
              ...
              >
                <span
                  className={`material-symbols-outlined text-[24px] transition-colors ${
                    isActive ? 'text-primary' : 'text-[#8d7b6f] group-hover:text-[#cba990]'
                  }`}
              ...
              <p
                className={`text-[10px] leading-none tracking-wide transition-colors ${
                  isActive
                    ...
                    : 'font-medium text-[#8d7b6f] group-hover:text-[#cba990]'
                }`}
              >
                {t(tab.labelKey)}
              </p>
